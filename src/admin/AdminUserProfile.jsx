...
import { addCommentAPI, addFlagcommentAPI, addadminFavPostAPI, dltfavAPI, dltlikeAPI, frndcountAPI, getPostCommentsAPI, getSearchNavigatePostsAPI, getUserPostsAPI, manageadminlikeAPI, removecommentAPI, searchfrndcountAPI } from '../../services/allAPI';
import { addResponseContext, commentdeleteContext, editResponseContext, favremoveContext, likecountResponseContext, likeremoveContext, postremoveResponseContext, userResponseContext } from '../Context/ContextAPI';
...
function AdminUserProfile({post}) {
  ...
  useEffect(() => {
    ...
    frndcount()
  }, [postData.image,addResponse,lgShow,MlShow,poststatusResponse,commentdlt,likeremove,]);
useEffect(()=>{

},[favremove,likecountResponse])
  ...
  const lusername = JSON.parse(sessionStorage.getItem('Name'))
  ...
  const luser = JSON.parse(sessionStorage.getItem('existingUser'))
  const luserId = luser._id
            ...
            {selectedPost && (
              <div className="row modallength">
                ...
                <div className="col-lg-6 text-light">
                  ...
                  <div className="d-flex justify-content-around w-25 pt-4">

                  <div className="d-flex justify-content-evenly">
                    {selectedPost.likes.some(like => like.lid === luserId)?(<i className={`fa-solid fa-heart text-danger fa-lg ic1 mt-2 likebtn `} onClick={() => dltike(selectedPost)} ></i>):(<i className={`fa-regular fa-heart fa-lg ic1 mt-2 likebtn` }  onClick={() => managelike(selectedPost)}></i>)}
                    
                  </div>
                  <div className="d-flex justify-content-evenly mt-2">
                  {selectedPost.fav.some(fav => fav.fid === luserId)?(<i className={`fa-solid fa-bookmark fa-lg `} onClick={() => handledltfav(selectedPost)}></i>):(<i className={`fa-regular fa-bookmark fa-lg bk`} onClick={() => handlefavPost(selectedPost)}></i>)}
